...
$(function() {
	...
	$('#level_1').change(function() {
		...
		$('#level_2 option').not('.default-option').each(function(i) {
			this.value = i;
		});
	});
	$('#level_2').change(function() {
		...
		$('#level_3 option').not('.default-option').each(function(i) {
			var format_4 = (i+1 + 10000).toString().slice(1);
	...
	$('#mLevel_1').change(function() {
		...
		$('#mLevel_2 option').not('.default-option').each(function(i) {
			this.value = i;
		});
	});
	...
	*/	
	function initSelector() {
		//初始化二级菜单
		var ml2_selectedIndex = parseInt($('#h-mLevel_2').val()) + 1;	
		$('#mLevel_1').trigger('change');		
		$('#mLevel_2 option').eq(ml2_selectedIndex).attr('selected', 'selected');		
		
		//初始化3级菜单第二层级
		var l2_selectedIndex = parseInt($('#h-level_2').val()) + 1;	
		$('#level_1').trigger('change');
		$('#level_2 option').eq(l2_selectedIndex).attr('selected', 'selected');		
		
		//初始化3级菜单第3层级		
		$('#level_2').trigger('change');
		var l3_selectedIndex = $('#h-level_3').val().slice(-2);
		$('#level_3 option').eq(l3_selectedIndex).attr('selected', 'selected');		
	}
